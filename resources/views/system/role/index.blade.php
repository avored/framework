...
@section('meta_title')
    {{ __('avored::system.list') }} {{ __('avored::system.role') }}: AvoRed E commerce Admin Dashboard
@endsection

@section('page_title')
    <div class="text-gray-800 flex items-center">
        <div class="text-xl text-red-700 font-semibold">
            {{ __('avored::system.list') }} {{ __('avored::system.role') }}
        </div>
        <div class="ml-auto">
...
<div class="block w-full" x-data="avoredTable('{{ request()->get('filter', '') }}')">
            ...
            <div x-on:click.away="filterBtnClicked=false" class="mb-2 relative flex sm:flex-row flex-col">
                <div class="block relative">
                    ...
                    <input placeholder="Search"
                        x-model="filterText"
                        x-on:change="filterData('{{ route('admin.role.index') }}', $event)"
                        ...
                        <td class="px-6 py-4 text-sm leading-5 border-b border-gray-200">
                            ...
                            <a href="{{ route('admin.role.destroy', $role) }}"
                                ...
                                onclick="event.preventDefault();
                                    document.getElementById('admin-role-{{ $role->id }}-delete').submit();">
                                {{ __('avored::system.delete') }}
                            </a>
                            <form id="admin-role-{{ $role->id }}-delete" 
    ...
    <div class="block mt-5 w-full">
        {{ $roles->render('avored::partials.paginate') }}
    </div>
</div>
@endsection
