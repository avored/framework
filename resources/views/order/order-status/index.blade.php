...
@section('meta_title')
    {{ __('avored::system.list') }} {{ __('avored::system.terms.order_status') }}: AvoRed E commerce Admin Dashboard
@endsection

@section('page_title')
    <div class="text-gray-800 flex items-center">
        <div class="text-xl text-red-700 font-semibold">
            {{ __('avored::system.list') }} {{ __('avored::system.terms.order_status') }}
        </div>
        <div class="ml-auto">
...
<div class="block w-full" x-data="avoredTable('{{ request()->get('filter', '') }}')">
            ...
            <div x-on:click.away="filterBtnClicked=false" class="mb-2 relative flex sm:flex-row flex-col">
                <div class="block relative">
                    ...
                    <input placeholder="Search"
                        x-model="filterText"
                        x-on:change="filterData('{{ route('admin.order-status.index') }}', $event)"
                ...
                @foreach ($orderStatuses as $orderStatus)
                    <tr>
                        ...
                        <td x-show="columns.name"  class="px-6 py-4 text-sm leading-5 border-b border-gray-200">
                            <a href="{{ route('admin.order-status.edit', $orderStatus) }}"
                                class="text-primary">
                                {{ $orderStatus->name }}
                            </a>
                        </td>
                        ...
                        <td class="px-6 py-4 text-sm leading-5 border-b border-gray-200">
                            ...
                            <a href="{{ route('admin.order-status.destroy', $orderStatus) }}"
                                class="text-primary"
                                onclick="event.preventDefault();
                                    document.getElementById('admin-order-status-{{ $orderStatus->id }}-delete').submit();">
                                {{ __('avored::system.delete') }}
                            </a>
                            <form id="admin-order-status-{{ $orderStatus->id }}-delete" 
                                action="{{ route('admin.order-status.destroy', $orderStatus) }}" 
    ...
    <div class="block mt-5 w-full">
        {{ $orderStatuses->render('avored::partials.paginate') }}
    </div>
</div>
@endsection
