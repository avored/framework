...
<div>
    <div class="p-5">
        <div class="flex w-full">
            ...
            <span class="ml-auto">
                <x-avored::link url="{{ route('admin.order-status.create') }}" style="button-primary">
                    ...
                    <x-slot name="body">
                        @foreach ($orderStatuses as $orderStatus)
                            <x-avored::table.row class="{{ ($loop->index % 2 == 0) ? '' : 'bg-gray-200'  }}">
                                ...
                                <x-avored::table.cell>
                                    <div class="flex">
                                        <x-avored::link url="{{ route('admin.order-status.edit', $orderStatus) }}">
                                        ...
                                        <x-avored::link
                                            x-on:click.prevent="toggleConfirmationDialog(
                                                true,
                                                {{ $orderStatus }},
                                                '{{ __('avored::system.confirmation_delete_message', ['attribute_value' => $orderStatus->name, 'attribute' => strtolower(__('avored::system.order-status'))]) }}',
                                                '{{ route('admin.order-status.destroy', $orderStatus) }}'
                                            )"
                                            url="{{ route('admin.order-status.destroy', $orderStatus) }}">
                                            <i class="w-5 h-5" data-feather="trash"></i>
                                            <x-avored::form.form
                                                id="category-destory-{{ $orderStatus->id }}"
                                                method="delete"
