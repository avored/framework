...
@section('meta_title')
    {{ __('avored::system.pages.title.create', ['attribute' => __('avored::system.terms.order_status')]) }}: AvoRed E commerce Admin Dashboard
@endsection

@section('page_title')
    <div class="text-gray-800 flex items-center">
        <div class="text-xl text-red-700 font-semibold">
            {{ __('avored::system.pages.title.create', ['attribute' => __('avored::system.terms.order_status')]) }}
        </div>
    </div>
@endsection
...
<div class="flex items-center">
    <order-status-save base-url="{{ asset(config('avored.admin_url')) }}" inline-template>
        <div class="w-full block">
            <form 
                method="post"
                action="{{ route('admin.order-status.store') }}"                    
                @submit="handleSubmit"
            >
                @csrf
                <avored-tabs>
