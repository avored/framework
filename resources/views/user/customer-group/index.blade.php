...
@section('meta_title')
    {{ __('avored::system.list') }} {{ __('avored::system.customer-group') }}: AvoRed E commerce Admin Dashboard
@endsection

@section('page_title')
    <div class="text-gray-800 flex items-center">
        <div class="text-xl text-red-700 font-semibold">
            {{ __('avored::system.list') }} {{ __('avored::system.customer-group') }}
        </div>
        <div class="ml-auto">
...
<div class="block w-full" x-data="avoredTable('{{ request()->get('filter', '') }}')">
            ...
            <div x-on:click.away="filterBtnClicked=false" class="mb-2 relative flex sm:flex-row flex-col">
                <div class="block relative">
                    ...
                    <input placeholder="Search"
                        x-model="filterText"
                        x-on:change="filterData('{{ route('admin.customer-group.index') }}', $event)"
                ...
                <div x-show="filterBtnClicked" class="absolute z-10 right-0" style="top:100%">
                    <div class="border-3 rounded-b text-white p-3 w-auto bg-gray-500 border-gray-800" style="top:100%;min-width: 10rem;">
                        <ul>
                            ...
                            <li class="z-50 py-2">
                                <input id="checkbox-column-is-default" 
                                    x-bind:checked="columns.is_default"
                                    x-on:click="toggleHiddenColumn('is_default')" 
                                    type="checkbox" />
                                <label for="checkbox-column-is-default" 
                                    class="ml-3 text-xs">
        ...
        <table class="min-w-full">
            <thead class="bg-gray-700 text-white">
                <tr>
                    ...
                    <th x-show="columns.is_default" class="px-6 py-3 border-b text-left border-gray-200 text-xs font-medium uppercase">
                        {{ __('avored::system.is_default') }}
                    </th>
                ...
                @foreach ($customerGroups as $customerGroup)
                    <tr>
                        ...
                        <td x-show="columns.name"  class="px-6 py-4 text-sm leading-5 border-b border-gray-200">
                            <a href="{{ route('admin.customer-group.edit', $customerGroup) }}"
                                class="text-primary">
                                {{ $customerGroup->name }}
                            </a>
                        </td>
                        <td x-show="columns.is_default" 
                        ...
                        <td class="px-6 py-4 text-sm leading-5 border-b border-gray-200">
                            ...
                            <a href="{{ route('admin.customer-group.destroy', $customerGroup) }}"
                                class="text-primary"
                                onclick="event.preventDefault();
                                    document.getElementById('admin-customer-group-{{ $customerGroup->id }}-delete').submit();">
                                {{ __('avored::system.delete') }}
                            </a>
                            <form id="admin-customer-group-{{ $customerGroup->id }}-delete" 
                                action="{{ route('admin.customer-group.destroy', $customerGroup) }}" 
    ...
    <div class="block mt-5 w-full">
        {{ $customerGroups->render('avored::partials.paginate') }}
    </div>
</div>
@endsection
