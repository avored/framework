...
@section('meta_title')
    {{ __('avored::system.pages.title.create', ['attribute' => __('avored::system.terms.user_group')]) }}: AvoRed E commerce Admin Dashboard
@endsection

@section('page_title')
    <div class="text-gray-800 flex items-center">
        <div class="text-xl text-red-700 font-semibold">
            {{ __('avored::system.pages.title.create', ['attribute' => __('avored::system.terms.user_group')]) }}
        </div>
    </div>
@endsection
...
<div class="flex items-center">
    <user-group-save base-url="{{ asset(config('avored.admin_url')) }}" inline-template>
        <div class="w-full block">
            <form method="post" action="{{ route('admin.user-group.store') }}">
                @csrf
                <avored-tabs>
                    @foreach ($tabs as $tab)
