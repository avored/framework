...
<div class="form-check">
    ...
    <input type="radio" name="shipping_option"  data-title="{{ $shippingOption->name() }}"  data-cost="{{ number_format($shippingOption->amount(),2) }}"
     id="{{ $shippingOption->identifier() }}"  value="{{ $shippingOption->identifier() }}"
    ...
    <label for="{{ $shippingOption->identifier() }}" class="form-check-label">
        @if(null === $shippingOption->amount())
            {{ $shippingOption->name() }}
        @else
            {{ $shippingOption->name() . " R$" . number_format($shippingOption->amount(),2, ',', '.') }}
        @endif
    </label>
    @if ($errors->has('shipping_option'))
