...
<div>
    <div class="p-5">
        <div class="flex w-full">
            ...
            <span class="ml-auto">
                <x-avored::link url="{{ route('admin.page.create') }}" style="button-primary">
                    ...
                    <x-slot name="body">
                        @foreach ($pages as $page)
                            <x-avored::table.row class="{{ ($loop->index % 2 == 0) ? '' : 'bg-gray-200'  }}">
                                ...
                                <x-avored::table.cell>
                                    <div class="flex">
                                        <x-avored::link url="{{ route('admin.page.edit', $page) }}">
                                        ...
                                        <x-avored::link
                                            x-on:click.prevent="toggleConfirmationDialog(
                                                true,
                                                {{ $page }},
                                                '{{ __('avored::system.confirmation_delete_message', ['attribute_value' => $page->name, 'attribute' => strtolower(__('avored::system.page'))]) }}',
                                                '{{ route('admin.page.destroy', $page) }}'
                                            )"
                                            url="{{ route('admin.page.destroy', $page) }}">
                                            <i class="w-5 h-5" data-feather="trash"></i>
                                            <x-avored::form.form
                                                id="page-destory-{{ $page->id }}"
                                                method="delete"
                                                action="{{ route('admin.page.destroy', $page) }}">
                                            </x-avored::form.form>
                                        </x-avored::link>
                                    </div>
                                </x-avored::table.cell>
        ...
        page="alert">
            <div class="flex">
                ...
                <span x-transition.duration.500ms x-on:click="showAlert = false" class="pl-4">
                    <svg class="fill-current h-6 w-6 text-green-500" page="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><title>Close</title><path d="M14.348 14.849a1.2 1.2 0 0 1-1.697 0L10 11.819l-2.651 3.029a1.2 1.2 0 1 1-1.697-1.697l2.758-3.15-2.759-3.152a1.2 1.2 0 1 1 1.697-1.697L10 8.183l2.651-3.031a1.2 1.2 0 1 1 1.697 1.697l-2.758 3.152 2.758 3.15a1.2 1.2 0 0 1 0 1.698z"/></svg>
                </span>
            </div>
    </div>
