...
@section('meta_title')
    {{ __('avored::system.list') }} {{ __('avored::system.menu-group') }}: AvoRed E commerce Admin Dashboard
@endsection

@section('page_title')
    <div class="text-gray-800 flex items-center">
        <div class="text-xl text-red-700 font-semibold">
            {{ __('avored::system.list') }} {{ __('avored::system.menu-group') }}
        </div>
        <div class="ml-auto">
...
<div class="block w-full" x-data="avoredTable('{{ request()->get('filter', '') }}')">
            ...
            <div x-on:click.away="filterBtnClicked=false" class="mb-2 relative flex sm:flex-row flex-col">
                <div class="block relative">
                    ...
                    <input placeholder="Search"
                        x-model="filterText"
                        x-on:change="filterData('{{ route('admin.menu-group.index') }}', $event)"
                ...
                <div x-show="filterBtnClicked" class="absolute z-10 right-0" style="top:100%">
                    <div class="border-3 rounded-b text-white p-3 w-auto bg-gray-500 border-gray-800" 
                        ...
                        <ul>
                            ...
                            <li class="z-50 py-2">
                                <input id="checkbox-column-identifier" 
                                    x-bind:checked="columns.identifier"
                                    x-on:click="toggleHiddenColumn('identifier')" 
                                    type="checkbox" />
                                <label for="checkbox-column-identifier" 
                                    class="ml-3 text-xs">
                                    {{ __('avored::system.identifier') }}
                                </label>
                            </li>
        ...
        <table class="min-w-full">
            <thead class="bg-gray-700 text-white">
                <tr>
                    ...
                    <th x-show="columns.identifier" class="px-6 py-3 border-b text-left border-gray-200 text-xs font-medium uppercase">
                        {{ __('avored::system.identifier') }}
                    </th>
                ...
                @foreach ($menuGroups as $menuGroup)
                    <tr>
                        ...
                        <td x-show="columns.name"  class="px-6 py-4 text-sm leading-5 border-b border-gray-200">
                            <a href="{{ route('admin.menu-group.edit', $menuGroup) }}"
                                class="text-primary">
                                {{ $menuGroup->name }}
                            </a>
                        </td>
                        <td x-show="columns.identifier" 
                        ...
                        <td class="px-6 py-4 text-sm leading-5 border-b border-gray-200">
                            ...
                            <a href="{{ route('admin.menu-group.destroy', $menuGroup) }}"
                                class="text-primary"
                                onclick="event.preventDefault();
                                    document.getElementById('admin-menu-group-{{ $menuGroup->id }}-delete').submit();">
                                {{ __('avored::system.delete') }}
                            </a>
                            <form id="admin-menu-group-{{ $menuGroup->id }}-delete" 
                                action="{{ route('admin.menu-group.destroy', $menuGroup) }}" 
    ...
    <div class="block mt-5 w-full">
        {{ $menuGroups->render('avored::partials.paginate') }}
    </div>
</div>
@endsection
