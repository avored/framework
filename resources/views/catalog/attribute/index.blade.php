...
@section('meta_title')
    {{ __('avored::system.attribute') }} {{ __('avored::system.list') }}: AvoRed E commerce Admin Dashboard
@endsection
...
<div class="block w-full" x-data="avoredTable('{{ request()->get('filter', '') }}')">
            ...
            <div x-on:click.away="filterBtnClicked=false" class="mb-2 relative flex sm:flex-row flex-col">
                <div class="block relative">
                    ...
                    <input placeholder="Search"
                        x-model="filterText"
                        x-on:change="filterData('{{ route('admin.attribute.index') }}', $event)"
                ...
                @foreach ($attributes as $attribute)
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm leading-5 border-b border-gray-200">
                            ...
                            <a href="{{ route('admin.attribute.destroy', $attribute) }}"
                                class="text-primary"
                                x-on:click.prevent="deleteOnClick('{{ json_encode($attribute) }}')"    
                            >
                                {{ __('avored::system.delete') }}
                            </a>
                            <form id="admin-attribute-{{ $attribute->id }}-delete" 
                                action="{{ route('admin.attribute.destroy', $attribute) }}" 
    ...
    <div class="block mt-5 w-full">
        {{ $attributes->render('avored::partials.paginate') }}
    </div>
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        ...
                        <div class="mt-2">
                            <p class="text-sm leading-5 text-gray-500">
                                {{ __('avored::system.delete_confirmation_message', ['attribute' => strtolower(__('avored::system.attribute'))]) }}
                            </p>
                        </div>
                    </div>
                </div>
...
<script>
    function avoredTable(filterText = '') {
        return {
            ...
            approveConfirmation() {
                document.getElementById(`admin-attribute-${this.modelResource.id}-delete`).submit();
            }, 
            filterData(url, e) {
