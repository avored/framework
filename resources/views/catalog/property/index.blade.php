...
@section('meta_title')
    {{ __('avored::system.property') }} {{ __('avored::system.list') }}: AvoRed E commerce Admin Dashboard
@endsection
...
<div class="block w-full" x-data="avoredTable('{{ request()->get('filter', '') }}')">
            ...
            <div x-on:click.away="filterBtnClicked=false" class="mb-2 relative flex sm:flex-row flex-col">
                <div class="block relative">
                    ...
                    <input placeholder="Search"
                        x-model="filterText"
                        x-on:change="filterData('{{ route('admin.property.index') }}', $event)"
                ...
                @foreach ($properties as $property)
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm leading-5 border-b border-gray-200">
                            ...
                            <a href="{{ route('admin.property.destroy', $property) }}"
                                class="text-primary"
                                x-on:click.prevent="deleteOnClick('{{ json_encode($property) }}')"
                            >
                                {{ __('avored::system.delete') }}
                            </a>
                            <form id="admin-property-{{ $property->id }}-delete" 
    ...
    <div class="block mt-5 w-full">
        {{ $properties->render('avored::partials.paginate') }}
    </div>
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        ...
                        <div class="mt-2">
                            <p class="text-sm leading-5 text-gray-500">
                                {{ __('avored::system.delete_confirmation_message', ['attribute' => strtolower(__('avored::system.property'))]) }}
                            </p>
                        </div>
                    </div>
                </div>
...
<script>
    function avoredTable(filterText = '') {
        return {
            ...
            approveConfirmation() {
                document.getElementById(`admin-property-${this.modelResource.id}-delete`).submit();
            },  
            filterData(url, e) {
