<template>
    <div>
        <avored-table
            ...
            :prev_page_url="customerGroups.prev_page_url"
            :next_page_url="customerGroups.next_page_url"
            :items="customerGroups.data"
            :filerable="true"
            @changeFilter="filterTableData"
        >
            <template slot="name" slot-scope="{ item }">
                <a
                    :href="`${baseUrl}/customer-group/${item.id}/edit`"
...
<script>
export default {
    props: ["baseUrl", "initCustomerGroups", "filterUrl"],
    data() {
        return {
            columns: [],
            customerGroups: []
    ...
    mounted() {
        ...
        this.customerGroups = this.initCustomerGroups
    },
    methods: {
        filterTableData(e) {
            let app = this;
            ...
            axios.post(this.filterUrl, data).then(response => {
                app.customerGroups = response.data;
            });
        },
        getEditUrl(record) {
        ...
        deleteOnClick(record) {
            ...
            this.$confirm({
                title:
                    "Do you Want to delete " + record.name + " customer-group?",
                okType: "danger",
                onOk() {
                    axios
                        .delete(url)
