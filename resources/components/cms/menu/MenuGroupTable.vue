<template>
    <div>
        <avored-table
            :columns="columns"
            :from="menuGroups.from"
            :to="menuGroups.to"
            :total="menuGroups.total"
            :prev_page_url="menuGroups.prev_page_url"
            :next_page_url="menuGroups.next_page_url"
            :items="menuGroups.data"
            :filerable="true"
            @changeFilter="filterTableData"
        >
            <template slot="name" slot-scope="{ item }">
                <a
                    :href="`${baseUrl}/menu-group/${item.id}/edit`"
...
<script>
export default {
    props: ["baseUrl", "initMenuGroups", "filterUrl"],
    data() {
        return {
            columns: [],
            menuGroups: []
    ...
    mounted() {
        this.columns = [
            ...
            {
                label: this.$t("system.identifier"),
                fieldKey: "identifier",
                visible: true
            },
            {
        ...
        this.menuGroups = this.initMenuGroups
    },
    methods: {
        filterTableData(e) {
            let app = this;
        ...
        deleteOnClick(record) {
            ...
            var app = this;
            this.$confirm({
                message: `Do you Want to delete ${record.name} menu-group?`,
                callback: () => {
                    axios
                        .delete(url)
                        .then(response => {
