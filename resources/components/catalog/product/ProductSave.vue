    ...
    methods: {
        ...
        createSingleVariationModal() {
            _.each(this.productAttributes, (attribute) => {
                ...
                this.singleVariationAttributeOptions[attribute.id][option.id] =
                    option.display_text;
                });
            });
        ...
        handleVariationBtnClick(e) {
            ...
            createSingleVariationSave() {
                ...
                axios({
                    ...
                    window.response = _.get(data, 'data')
                    if (response.CreateProductVariation === null) {
                        const errors = _.get(data, 'errors')
                        _.each (errors, (error) => {
                            if (_.get(error, 'extensions.category') === 'validation') {
                                const errorMessages = _.get(error, 'extensions.validation.single_variation_attributes')
                                _.each (errorMessages, (errorMessage) => {
                                    alert("Fix me: " + errorMessage)
                                })
                            }
                        })
                    } else {
