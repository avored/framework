<template>
    <div class="mt-3">
         <avored-table
            :columns="columns"
            :from="initOrderStatuses.from"
            :to="initOrderStatuses.to"
            :total="initOrderStatuses.total"
            :prev_page_url="initOrderStatuses.prev_page_url"
            :next_page_url="initOrderStatuses.next_page_url"
            :items="initOrderStatuses.data"
        >
          <template slot="name" slot-scope="{item}">
              <a :href="`${baseUrl}/promotion-code/${item.id}/edit`" class="text-red-700 hover:text-red-600">
                  {{ item.name }}
              </a>
          </template>
...
export default {
  ...
  methods: {
      ...
      deleteOnClick(record) {
        ...
        var app = this;
        this.$confirm({message: `Do you Want to delete ${record.name} order-status?`, callback: () => {
           axios.delete(url)
              .then(response =>  {
